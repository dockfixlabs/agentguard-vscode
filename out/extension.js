...
function applyFindings(uri, findings) {
    const diagnostics = findings.map(f => {
        const range = new vscode.Range(Math.max(0, f.line - 1), f.column || 0, Math.max(0, f.line - 1), (f.column || 0) + 200);
        const severity = {
        ...
        const diag = new vscode.Diagnostic(range, `[${f.severity}] ${f.rule_name}: ${f.description}`, severity);
...
function showRules() {
    ...
    exec(`${python} -c "from agentguard.rules import ALL_RULES; [print(f'{r.rule_id} | {r.rule_name} | {r.severity.value} | {r.owasp.value if r.owasp else \"N/A\"}') for r in ALL_RULES]"`, (err, stdout) => {
        if (stdout) {
            const panel = vscode.window.createWebviewPanel('agentguardRules', 'AgentGuard Detection Rules', vscode.ViewColumn.One, {});
